...
CREATE TABLE Persona (
	...
	-- controllare lunghezza?
	n_telefono decimal(11,0)
...
CREATE TABLE Finanziamento (
	...
	entità decimal(9,0) NOT NULL,
	CHECK(entità>0)
);

CREATE TABLE Ciclo (
	tipo varchar(20) PRIMARY KEY,
	-- ciclo IN (primo, secondo)
...
CREATE TABLE Classe (
	...
	scuola char(8) REFERENCES Scuola (cm) 
					  ON UPDATE CASCADE,
	-- controllare che abbia ruolo "docente"
...
CREATE TABLE Studente(
	...
	scuola char(8) NOT NULL,
	FOREIGN KEY (classe, scuola) REFERENCES Classe (nome, scuola)
);
...
CREATE TABLE UsoSpecie (
	...
	scopo varchar(15),
	-- vedere come organizzare
	esposizioni varchar(15) NOT NULL,
...
CREATE TABLE DatiSchedaArduino (
	...
	-- da vedere come organizzare i range
...
CREATE TABLE Rilevazione (
	n_rilevazione decimal(3,0),
	specie varchar(20) NOT NULL,
	tipo_colt varchar(14) NOT NULL,
	scopo varchar(15) NOT NULL,
	timestamp_inserimento timestamp NOT NULL
									DEFAULT CURRENT_TIMESTAMP,
	tipo_substrato varchar(20),
	...
	FOREIGN KEY (specie, tipo_colt, scopo) 
		REFERENCES UsoSpecie (specie, tipo_colt, scopo),
	PRIMARY KEY (n_rilevazione, specie, tipo_colt, scopo),
	CHECK((resp_inserimento IS NULL) OR (resp_rilevazione != resp_inserimento)),
	CHECK(n_rilevazione>0),
	CHECK(tipo_substrato IN ('terriccio da rinvaso','suolo pre-esistente')),
	CHECK((cosa_monitorato IS NULL) OR ((cosa_monitorato IN ('suolo','aria')) AND (scopo = 'fitobonifica')))
);

CREATE TABLE Misurazione (
	id decimal(6,0) PRIMARY KEY,
	n_rilevazione integer NOT NULL, --UNIQUE e not NULL servono entambi?
	specie varchar(20) NOT NULL,
	tipo_colt varchar(14) NOT NULL,
	scopo varchar(15) NOT NULL,
	...
	n_foglie_danneggiate decimal(3,0) NOT NULL,
	n_frutti decimal(3,0) NOT NULL,
	n_fiori decimal(3,0) NOT NULL,
	...
	FOREIGN KEY (n_rilevazione, specie, tipo_colt, scopo)
		REFERENCES Rilevazione (n_rilevazione, specie, tipo_colt, scopo),
	FOREIGN KEY (n_replica, gruppo)
		REFERENCES Replica (id, gruppo),
	UNIQUE (n_rilevazione, specie, tipo_colt, scopo, n_replica, gruppo),
	CHECK(id >0 AND
	umidità >0 AND
	pH >0 AND
...
/*INSERT INTO Persona VALUES('FSKCOBEOYHSOSIDC', 'Carlo', 'Panuozzi', 'alcmeomfinve', 'studente', 10863902);*/
...
BEGIN
	...
	/* Sono già assegnati ad una altro gruppo? */
